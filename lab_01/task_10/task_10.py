...
# Вывести на экран уникальные элементы каждого списка и общие
print("\nУникальные элементы:")
print(sa)
print(sb)
...
# Если использовать sum() можно, то код выглядит так:
sr_ar1 = sum(c[1::2])/len(c[1::2])
# Если использовать sum() нельзя, то код выглядит так с учётом того, что всего в списке на данный момент 8 элементов:
sr_ar2 = (c[1]+c[3]+c[5]+c[7])/len(c[1::2])

# Среднее геометрическое элементов списка 'c', расположенных на нечетных местах
# При условии, что использовать if, for, while, try, import и другие конструкции языка нельзя, то и использование функций для перемножения чисел (аналог. sum() в прошлом пункте )не предоставляется возможным.
sr_geom = round((c[0]*c[2]*c[4]*c[6])**(1/len(c[0::2])), 2)
...
print("\nИтоговые:")
print("3-й:", c)
print("отсортированный по возрастанию: ", c_asc)
print("отсортированный по убыванию: ", c_desc)
print("Среднее арифметическое с использованием sum(): ", sr_ar1)
print("Среднее арифметическое без использования sum(): ", sr_ar2)
print("Среднее геометрическое: ", sr_geom)
print("Максимальное: ", c_max)
print("Минимальное: ", c_min)

# Пример вывода по изначальному условию:
#
# Исходные списки:
